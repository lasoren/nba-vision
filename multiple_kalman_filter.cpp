...
MultipleKalmanFilter::MultipleKalmanFilter(const int& num_objects, const vector<pair<int, int>>& object_locations) {
	kalman_filters_ = map<int, KalmanFilter>();
	for (int i = 0; i < num_objects; ++i) {
		kalman_filters_[i] = KalmanFilter(kNumDynamicParams, kNumMeasurementParams);
		KalmanFilter& kalman_filter = kalman_filters_[i];
		InitKalmanFilter(kalman_filter, object_locations[i].first, object_locations[i].second);
	}
}

Mat MultipleKalmanFilter::CorrectAndPredictForObject(const int& object_idx, const Mat_<float>& measurement) {
	if (kalman_filters_.find(object_idx) == kalman_filters_.end()) {
		kalman_filters_[object_idx] = KalmanFilter(kNumDynamicParams, kNumMeasurementParams);
		return InitKalmanFilter(kalman_filters_[object_idx], measurement(0), measurement(1));
	}
...
Mat MultipleKalmanFilter::InitKalmanFilter(KalmanFilter& kalman_filter, const float& init_x, const float& init_y) {
	// Represents position_x, position_y, velocity_x, velocity_y, acceleration_x and acceleration_y and how they transition between states.
